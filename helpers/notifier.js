  ...
  function getUser() {
    ...
    return new Promise((resolve, reject) => {
      robot.adapter.client.web.users.list((err, res) => {
        ...
        if (!res.ok) {
          return reject(
            new Error("An error has occurred while fetching users list.")
          );
        }
  ...
  function getChannel(user) {
    ...
    return new Promise((resolve, reject) => {
      robot.adapter.client.web.im.list((err, res) => {
        ...
        if (!res.ok) {
          return reject(
            new Error("An error has occurred while fetching im list.")
          );
        }
        ...
        if (!Im) {
          return reject(
            new Error(`User ${username} was not found in im list.`)
          );
        }
